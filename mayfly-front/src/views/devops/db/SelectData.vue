      ...
      <div class="fl">
        ...
        >
          <el-option
            v-for="item in dbs"
            :key="item.id"
            :label="`${item.name} [${dbTypeName(item.type)}]`"
            :value="item.id"
          >
          </el-option>
        </el-select>
...
export default class SelectData extends Vue {
  ...
  dbTypeName(type: any) {
    return enums.DbTypeEnum.getLabelByValue(type)
  }
  ...
  changeDb(id: number) {
    ...
    dbApi.tableMetadata.request({ id }).then((res) => {
      ...
      if (this.tableMetadata.length > 0) {
        this.tableName = this.tableMetadata[0]['name']
    ...
    dbApi.hintTables
      ...
      .then((res) => {
        this.cmOptions.hintOptions.tables = res
      })
  ...
  formatSql() {
    ...
    if (selectSql != '') {
      this.codemirror.replaceSelection(sqlFormatter.format(selectSql))
    } else {
      /* 将sql内容进行格式后放入编辑器中*/
      this.codemirror.setValue(sqlFormatter.format(this.sql))
    }
  }
