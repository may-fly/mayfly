  ...
  export default {
    ...
    data() {
      return {
        ...
        addFile: permission.machine.addFile,
        delFile: permission.machine.delFile,
    ...
    methods: {
      getFiles() {
        Req.request('Get', '/sys/machines/' + this.machineId + '/files').then(res => {
          this.fileTable = res
        })
      },
      /**
      ...
      addFiles(row) {
        ...
        this.addFile.request(row).then(res => {
      ...
      updateContent() {
        this.updateFileContent.request({
