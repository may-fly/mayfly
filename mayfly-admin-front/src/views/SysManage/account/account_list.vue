  ...
  <div class="role-list">
    ...
    <el-table :data="datas" border ref="table" style="width: 100%" @current-change="choose">
      ...
      <el-table-column prop="status" label="角色">
        <template slot-scope="scope">
          <el-tag v-for="role in scope.row.roles" :key="role.id" :type="role.status === 1 ? 'success' : 'danger'">{{role.name}}</el-tag>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async search() {
        let res = await this.permission.list.request(this.query);
        this.datas = res.list;
