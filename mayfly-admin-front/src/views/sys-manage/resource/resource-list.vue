<template>
  <div class="menu">
    <ToolBar>
      <div><span style="font-size: 14px;"><i class="el-icon-info"></i>红色字体表示禁用状态</span></div>
    ...
    <el-tree class="none-select" :indent="38" node-key="id" :props="props" :data="data" @node-expand="handleNodeExpand"
      ...
      <span class="custom-tree-node" slot-scope="{ node, data }">
        ...
        <el-link @click="info(data)" style="margin-left: 25px;" icon="el-icon-view" type="info" :underline="false" />

        <el-link v-permission="permission.update.code" @click="editResource(data)" type="primary" icon="el-icon-edit"
          :underline="false" />

        <el-link v-permission="permission.save.code" @click="addResource(data)" v-if="data.type === enums.ResourceTypeEnum.MENU.value"
          icon="el-icon-circle-plus-outline" :underline="false" type="success" />

        <el-link v-permission="permission.changeStatus.code" @click="changeStatus(data, 0)" v-if="data.status === 1 && data.type === enums.ResourceTypeEnum.PERMISSION.value"
          icon="el-icon-circle-close" :underline="false" type="warning" />

        <el-link v-permission="permission.changeStatus.code" @click="changeStatus(data, 1)" v-if="data.status === 0 && data.type === enums.ResourceTypeEnum.PERMISSION.value"
          type="success" icon="el-icon-circle-check" :underline="false" plain />

        <el-link v-permission="permission.del.code" v-if="data.children == null" @click="deleteMenu(data)" type="danger"
          icon="el-icon-remove-outline" :underline="false" plain />
      </span>
    </el-tree>
  ...
  export default {
    ...
    methods: {
      ...
      async editResource(data) {
        ...
        this.dialogForm.data = await this.permission.detail.request({id: data.id});;
      ...
      async info(data) {
        ...
        if (info.type === enums.ResourceTypeEnum.MENU.value) {
          this.$alert('<strong style="margin-right: 18px">名称:</strong>' + info.name +
            ...
            ' <br/><br/><strong style="margin-right: 18px">路径:</strong>' + (info.code ? info.code : "无") +
