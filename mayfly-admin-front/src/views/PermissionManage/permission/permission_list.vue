<template>
  <div class="user-list">
    ...
    <el-table :data="permissionList" border ref="table" max-height="800" style="width: 100%">
      ...
      <el-table-column min-width="45" prop="method" label="调用方法">
        <div slot-scope="scope" style="width: 100%;">
          <el-tag v-if="scope.row.method == requestMethod.GET.value">GET</el-tag>
          <el-tag type="success" v-if="scope.row.method == requestMethod.POST.value">POST</el-tag>
          <el-tag type="warning" v-if="scope.row.method == requestMethod.PUT.value">PUT</el-tag>
          <el-tag type="danger" v-if="scope.row.method == requestMethod.DELETE.value">DELETE</el-tag>
        </div>
      </el-table-column>
      ...
      <el-table-column label="操作" width="270">
        <template slot-scope="scope">
          <el-button v-permission="permission.changeStatus.code" v-if="scope.row.status == 0" @click="changeStatus(scope.row, 1)"
            type="success" :ref="scope.row.id" size="small">启用</el-button>
          <el-button v-permission="permission.changeStatus.code" v-if="scope.row.status == 1" @click="changeStatus(scope.row, 0)"
            type="danger" size="small">禁用</el-button>
          <el-button v-permission="permission.del.code" v-if="scope.row.status != 3" @click="deleteApi(scope.row.id)"
            type="danger" size="small" icon="el-icon-delete">删除</el-button>
    ...
    <PermissionEdit :title="dialogForm.title" :visible="dialogForm.visible" :data="dialogForm.data" @cancel="cancel"
...
<script>
  ...
  export default {
    data() {
      return {
        requestMethod: enums.requestMethod,
    ...
    methods: {
      ...
      deleteApi(id) {
        ...
        this.permission.del.request({
          ...
          this.permission.search.request(null).then(res => {
      ...
      search() {
        ...
        if (!permission.show) {
          ...
          searchPermission.request(this.query).then(res => {
